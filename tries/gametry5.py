...
def draw_grid():
    ...
    start_x = 100  # starting position for the grid
    start_y = 100

    for row in range(grid_height):
        for col in range(grid_width):
            # Draw the grid rectangle for each cell
            pygame.draw.rect(SCREEN, OUTLINE, (start_x + col * (LETTER_SIZE + spacing),
                                               start_y + row * (LETTER_SIZE + spacing),
                                               LETTER_SIZE, LETTER_SIZE), 3)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                if game_result != "":
                    ...
                    if len(current_guess_string) == 5 and current_guess_string.lower() in game_words:
